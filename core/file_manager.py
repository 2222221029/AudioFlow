...
5. **同名章节分段括号化**：分析所有文件，若多个文件的章节标题去掉结尾的数字/中文数字后完全相同（即同一章节被拆分为多集），则将结尾的数字转为中文并加上全角括号。数字对应：1/一→（一），2/二→（二），3/三→（三），4/四→（四），5/五→（五）以此类推。例："醋意十足一"和"醋意十足二"→分别输出"醋意十足（一）""醋意十足（二）"；若某标题结尾有数字但整个列表中只有这一个（无同组其他集），则不加括号直接保留原数字
