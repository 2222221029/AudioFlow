...
DEFAULT_TEMPLATES = [
    ...
    {"id": "t5", "name": "纯序号",                    "template": "{chapter_index_4}.{ext}"},
    {"id": "t6", "name": "第N章 章节名",              "template": "第{chapter_index_3}章 {chapter_title}.{ext}"},
]
...
4. **剥离章节序号**：chapter_title 只保留正文标题，去掉开头的独立数字序号（含"集/章/回/话/期"等单位词）。例："1离家"→"离家"，"003集 酣睡"→"酣睡"，"第5章破晓"→"破晓"。**注意：保留标题中已有括号的分段标记，如"（一）""（二）"**。例："玄门炼真 （一）"→"玄门炼真 （一）"（序号由模板变量单独控制，不要留在标题里）
5. **同名章节分段括号化**：若多个文件的章节标题去掉结尾的数字/中文数字后完全相同，**且这些文件的文件名开头下载序号是连续的**（如 0021、0022 连续），才将结尾数字转为中文并加全角括号。数字对应：1/一→（一），2/二→（二），3/三→（三），4/四→（四），5/五→（五）以此类推。例：0021-xxx-醋意十足一 与 0022-xxx-醋意十足二 序号连续→分别输出"醋意十足（一）""醋意十足（二）"。若序号不连续（中间有其他章节间隔），则不加括号，直接按规则4处理（去掉结尾数字或保留）
