...
5. **同名章节分段括号化**：若多个文件的章节标题去掉结尾的数字/中文数字后完全相同，**且这些文件的文件名开头下载序号是连续的**（如 0021、0022 连续），才将结尾数字加上全角括号（**保持原有数字形式，不转换**）。例：0021-醋意十足一 与 0022-醋意十足二 →"醋意十足（一）""醋意十足（二）"；0021-醋意十足1 与 0022-醋意十足2 →"醋意十足（1）""醋意十足（2）"。若序号不连续（中间有其他章节间隔），则不加括号，直接按规则4处理（去掉结尾数字或保留）
