...
4. **剥离章节序号**：chapter_title 只保留正文标题，去掉开头的独立数字序号（含"集/章/回/话/期"等单位词）。例："1离家"→"离家"，"003集 酣睡"→"酣睡"，"第5章破晓"→"破晓"。**注意：保留标题中表示分段的中文数字括号，如"（一）""（二）"，这些是标题的一部分而非序号**。例："玄门炼真 （一）"→"玄门炼真 （一）"（序号由模板变量单独控制，不要留在标题里）
