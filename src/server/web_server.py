...
def _run_subscription_check(sid, queue_missing=False, source="subscription-check", progress=None, retry_restricted=False):
    ...
    with subscription_manager.locked():
        ...
        print('[DIAG] diff missing=' + str(len(diff.get("missing") or [])) + ' remote_total=' + str(diff.get("remote_total", 0)) + ' restricted=' + str(diff.get("restricted_count", 0)) + ' file_missing=' + str(diff.get("file_missing_count", 0)) + ' new=' + str(diff.get("new_count", 0)) + ' saved=' + str(diff.get("saved_total", 0)))
