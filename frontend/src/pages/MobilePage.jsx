...
const VIEW_META = {
  ...
  library: {title: '书库管理', subtitle: '扫描书库、刮削元数据、智能重命名', icon: 'i-folder', parent: 'more'},
...
function MoreView({app, installable, switchView}) {
  ...
  const cards = [
    ...
    {id: 'library', icon: 'i-folder', title: '书库管理', sub: '扫描书库、刮削元数据、智能重命名'},
...
function RoutedContent({app, installable, switchView, searchAndShowResults}) {
  ...
  if (mobileView === 'library') return <section className="view native-view active"><LibraryPage /></section>;
