...
“系统设置 → 自动重命名（仅手动下载）”默认启用安全确认模式，不需要增加 Compose 环境变量。网页或企业微信发起的手动下载完整成功后，AudioFlow 会按有声书技能规则分析整本专辑，生成持久化映射并通过通知渠道请求集中确认；订阅检查、自动订阅及其重试不会进入整理流程。
...
整理计划弹窗中的“整理本地文件夹”会扫描挂载的下载目录，列出最多三层、直接含音频文件的专辑目录。选择目录即可生成手动计划；路径经过下载根目录校验，重复请求会复用未完成计划。计划执行完成后会显示目标格式、广告残留、结尾标识、缺集预留空号、章节分隔和重名检查结果，验证失败只报告，不撤销已完成的改名。
...
计划支持两档 AI 辅助：`AI 复核风险项` 只分析规则引擎标出的存疑文件；`全量 AI 清洗` 按约 40 条一批顺序处理全部章节，显示进度并支持失败续跑。全量结果会展示原始文件名、规则标题、AI 标题、理由和置信度，默认勾选有改动的条目；应用建议后仍需用户最终确认，AI 不会直接执行文件操作。
